...
function buildWhere(filters) {
    ...
    if (confIds?.length)
        where.confidenceLevelId = { in: confIds };
...
exports.analyticsService = {
    async getDashboardSummary(filters) {
        ...
        const totalPipelineTcv = all
            .filter((o) => !['SECURED', 'O(H)', 'O(L)'].includes(o.dealStage.code))
        ...
        const closingThisQuarter = all.filter((o) => o.estimatedClosureDate && o.estimatedClosureDate <= qEnd).length;
        const wonCount = all.filter((o) => o.dealStage.code === 'SECURED').length;
        const lostCount = all.filter((o) => o.dealStage.code === 'O(L)').length;
    ...
    async getSubcategoryAnalytics(filters) {
        const where = buildWhere(filters);
        const groups = await prisma.opportunity.groupBy({
    ...
    async getSubcategoryByBUAnalytics(filters) {
        const where = buildWhere(filters);
        const groups = await prisma.opportunity.groupBy({
        ...
        for (const g of groups) {
            ...
            if (!pivot[subcatName])
                pivot[subcatName] = { rowLabel: subcatName };
    ...
    async getConfidenceAnalytics(filters) {
        const where = buildWhere(filters);
        const groups = await prisma.opportunity.groupBy({
    ...
    async getStageAnalytics(filters) {
        const where = buildWhere(filters);
        const groups = await prisma.opportunity.groupBy({
            by: ['dealStageId'],
            where,
    ...
    async getCustomerCategoryAnalytics(filters) {
        const where = buildWhere(filters);
        const groups = await prisma.opportunity.groupBy({
        ...
        for (const g of groups) {
            ...
            if (!pivot[custName])
                pivot[custName] = { rowLabel: custName };
    ...
    async getCountAnalytics(filters) {
        const where = buildWhere(filters);
    ...
    async getDashboardCharts(filters) {
        const [buData, categoryData, stageData] = await Promise.all([
            exports.analyticsService.getBUAnalytics(filters),
            exports.analyticsService.getCategoryAnalytics(filters),
            exports.analyticsService.getStageAnalytics(filters),
        ]);
