...
const buildWhere = (filters: any): { clause: string; params: any[] } => {
  const conds: string[] = ['o.is_active = true'];
  const params: any[] = [];
  let idx = 1;

  const arr = (v: any) => (Array.isArray(v) ? v : v ? [v] : null);
  ...
  if (customerIds?.length) { conds.push(`o.customer_id = ANY($${idx}::uuid[])`); params.push(customerIds); idx++; }
  if (businessUnitIds?.length) { conds.push(`o.business_unit_id = ANY($${idx}::uuid[])`); params.push(businessUnitIds); idx++; }
  if (productCategoryIds?.length) { conds.push(`o.product_category_id = ANY($${idx}::uuid[])`); params.push(productCategoryIds); idx++; }
  if (dealStageIds?.length) { conds.push(`o.deal_stage_id = ANY($${idx}::uuid[])`); params.push(dealStageIds); idx++; }
  if (confidenceLevelIds?.length) { conds.push(`o.confidence_level_id = ANY($${idx}::uuid[])`); params.push(confidenceLevelIds); idx++; }
  if (salesIds?.length) { conds.push(`o.pin_sales_id = ANY($${idx}::uuid[])`); params.push(salesIds); idx++; }
  if (filters.fromDate) { conds.push(`o.estimated_closure_date >= $${idx}`); params.push(new Date(filters.fromDate)); idx++; }
  if (filters.toDate) { conds.push(`o.estimated_closure_date <= $${idx}`); params.push(new Date(filters.toDate)); idx++; }

  return { clause: conds.join(' AND '), params };
};

const raw = <T>(sql: string, params: any[]): Promise<T[]> =>
...
export const getCategoryWise = async (filters: any) => {
  const { clause, params } = buildWhere(filters);
  return raw<any>(`
...
export const getSubcategoryWise = async (filters: any) => {
  const { clause, params } = buildWhere(filters);
  return raw<any>(`
...
export const getSubcategoryByBU = async (filters: any) => {
  const { clause, params } = buildWhere(filters);
  return raw<any>(`
...
export const getConfidenceLevel = async (filters: any) => {
  const { clause, params } = buildWhere(filters);
  return raw<any>(`
...
export const getBUWise = async (filters: any) => {
  const { clause, params } = buildWhere(filters);
  return raw<any>(`
...
export const getStageWise = async (filters: any) => {
  const { clause, params } = buildWhere(filters);
  return raw<any>(`
...
export const getCustomerWise = async (filters: any) => {
  const { clause, params } = buildWhere(filters);
  return raw<any>(`
...
export const getCustomerByCategory = async (filters: any) => {
  const { clause, params } = buildWhere(filters);
  return raw<any>(`
...
export const getTeamMembers = async (filters: any) => {
  const { clause, params } = buildWhere(filters);
  return raw<any>(`
...
export const getOpportunityCount = async (filters: any) => {
  const { clause, params } = buildWhere(filters);
  return raw<any>(`
